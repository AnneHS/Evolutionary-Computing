...
def survival_selection(whole_pop, whole_pop_fit):
    ...
    best = np.argmax(whole_pop_fit)
    chosen = np.random.choice(whole_pop.shape[0], N, p=probs, replace=False)[:-1]
...
def stuck(gen_without_improvement, last_solution, pop_fitness):
    best = np.argmax(pop_fitness)
    if last_solution == pop_fitness[best]:
        gen_without_improvement += 1
    else:
        gen_without_improvement = 0
    if gen_without_improvement == 5:
        return True
...
def evolution():
    ...
    for i in range(ini_g + 1, gens):
        ...
        if stuck(gen_without_improvement, last_solution, pop_fitness):
            pop, pop_fitness = doomsday(pop, pop_fitness)
